...
export class SkillCoursesService {
    ...
    async createCourse(data: { title: string; description: string; tags: string; thumbnail?: string; createdById: string }) {
    ...
    async createModule(courseId: string, data: { title: string; orderIndex: number }) {
        return this.prisma.skillModule.create({
            data: { ...data, courseId }
        });
    }

    async createLesson(moduleId: string, data: { title: string; content: string; videoUrl?: string; orderIndex: number }) {
