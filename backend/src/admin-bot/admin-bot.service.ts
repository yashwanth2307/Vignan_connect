...
export class AdminBotService {
  ...
  parseCommand(message: string): ParsedCommand {
    ...
    const batchSemRegex =
      /(?:update|promote|move|set)\s+(?:batch\s+)?(\d{4})\s*[-–]\s*(\d{4})\s+(?:batch\s+)?(?:to\s+)?(?:semester|sem)\s+(\d+)/i;
    ...
    const batchDeptSemRegex =
      /(?:update|promote|move|set)\s+(?:batch\s+)?(\d{4})\s*[-–]\s*(\d{4})\s+(\w+)\s+(?:batch\s+)?(?:to\s+)?(?:semester|sem)\s+(\d+)/i;
    ...
    const countBatchRegex =
      /(?:how many|count|list)\s+students?\s+(?:in\s+)?(?:batch\s+)?(\d{4})\s*[-–]\s*(\d{4})/i;
  ...
  async processCommand(message: string) {
    ...
    switch (cmd.action) {
      case 'UPDATE_BATCH_SEMESTER': {
        if (!cmd.batchStart || !cmd.batchEnd || !cmd.targetSemester) {
          throw new BadRequestException(
            'Could not parse batch years or target semester',
          );
        }
        ...
        if (cmd.departmentCode) {
          ...
          if (!dept)
            throw new BadRequestException(
              `Department ${cmd.departmentCode} not found`,
            );
      ...
      case 'LIST_BATCHES': {
        ...
        const batches = students.map(
          (s) => `${s.batchStartYear}-${s.batchEndYear}`,
        );
