...
export class PlacementsService {
  ...
  async applyToDrive(driveId: string, userId: string, resume?: string) {
    ...
    if (!drive || !drive.isActive)
      throw new NotFoundException('Drive not found or closed');

    // Check deadline
    if (new Date() > drive.deadline)
      throw new ForbiddenException('Application deadline has passed');
    ...
    if (
      !eligibleBranches.includes(student.department.code.toUpperCase()) &&
  ...
  async updateApplicationStatus(
    ...
    const app = await this.prisma.placementApplication.findUnique({
  ...
  async getDriveStats() {
    const [totalDrives, activeDrives, totalApplications, selectedCount] =
      await Promise.all([
