...
export class GroupsController {
    ...
    create(@Body() dto: { name: string; description?: string; courseOfferingId?: string }, @Req() req: any) {
    ...
    addMembers(@Param('id') id: string, @Body() body: { studentIds: string[] }, @Req() req: any) {
    ...
    addSection(@Param('id') id: string, @Param('sectionId') sectionId: string, @Req() req: any) {
    ...
    removeMember(@Param('id') id: string, @Param('studentId') studentId: string, @Req() req: any) {
    ...
    getMessages(
        ...
        return this.service.getMessages(id, req.user.sub, cursor, limit ? parseInt(limit) : 50);
    }
    ...
    createAssignment(
        @Param('id') id: string,
        @Body() dto: { title: string; description: string; dueAt: string; maxPoints?: number },
        @Req() req: any,
    ) {
    ...
    getAssignmentDetail(@Param('assignmentId') assignmentId: string, @Req() req: any) {
    ...
    getMySubmission(@Param('assignmentId') assignmentId: string, @Req() req: any) {
    ...
    runPlagiarismCheck(@Param('assignmentId') assignmentId: string, @Req() req: any) {
    ...
    @Roles(UserRole.FACULTY, UserRole.HOD)
    @ApiOperation({ summary: 'Review submission — verify or flag, award V-Points' })
    reviewSubmission(
        @Param('submissionId') submissionId: string,
        @Body() dto: { status: 'VERIFIED' | 'FLAGGED'; vPointsAwarded?: number; facultyRemarks?: string },
        @Req() req: any,
    ) {
