...
export class GroupsService {
    ...
    async createGroup(dto: {
        ...
        if (!faculty) throw new ForbiddenException('Only faculty can create groups');
        ...
        if (role === 'STUDENT') {
            ...
            return this.prisma.group.findMany({
                ...
                include: {
                    ...
                    _count: { select: { members: true, messages: true, assignments: true } },
                },
    ...
    async updateGroup(groupId: string, dto: { name?: string; description?: string; isActive?: boolean }, userId: string) {
    ...
    async getMessages(groupId: string, userId: string, cursor?: string, limit = 50) {
        await this.verifyGroupAccess(groupId, userId);
    ...
    async createAssignment(groupId: string, dto: {
        title: string;
        description: string;
        dueAt: string;
        maxPoints?: number;
    ...
    async submitAssignment(assignmentId: string, dto: {
        content: string;
        fileUrl?: string;
    }, userId: string) {
        const student = await this.prisma.student.findUnique({ where: { userId } });
        ...
        const isMember = await this.prisma.groupMember.findUnique({
            where: { groupId_studentId: { groupId: assignment.groupId, studentId: student.id } },
        });
    ...
    async runPlagiarismCheck(assignmentId: string, userId: string) {
        ...
        if (!faculty || assignment.group.facultyId !== faculty.id) {
            throw new ForbiddenException('Only the group faculty can run plagiarism checks');
        }
        ...
        if (submissions.length < 2) {
            return { message: 'Need at least 2 submissions to run plagiarism check', results: [] };
        }
    ...
    private calculateSimilarity(text1: string, text2: string): number {
        ...
        for (let i = 0; i <= norm1.length - 3; i++) charGrams1.add(norm1.slice(i, i + 3));
        for (let i = 0; i <= norm2.length - 3; i++) charGrams2.add(norm2.slice(i, i + 3));
    ...
    async reviewSubmission(submissionId: string, dto: {
        ...
        if (!faculty || submission.assignment.group.facultyId !== faculty.id) {
            throw new ForbiddenException('Only the group faculty can review submissions');
        }

        const vPoints = dto.status === 'VERIFIED' ? (dto.vPointsAwarded || submission.assignment.maxPoints) : 0;
    ...
    async getSubmissionStats(assignmentId: string) {
        ...
        return {
            ...
            totalVPointsAwarded: submissions.reduce((sum, s) => sum + s.vPointsAwarded, 0),
        };
    }
