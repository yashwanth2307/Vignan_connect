...
export class RegulationsService {
    ...
    private parseRulesJson(reg: any) {
        if (!reg) return reg;
        try {
            return { ...reg, rulesJson: typeof reg.rulesJson === 'string' ? JSON.parse(reg.rulesJson) : reg.rulesJson };
        } catch {
    ...
    async create(dto: CreateRegulationDto) {
        const existing = await this.prisma.regulation.findUnique({ where: { code: dto.code } });
        ...
        const reg = await this.prisma.regulation.create({
            data: { code: dto.code, rulesJson: rulesJsonStr, activeFrom: new Date(dto.activeFrom) },
        });
        return this.parseRulesJson(reg);
    }

    async findAll() {
        const regs = await this.prisma.regulation.findMany({ orderBy: { activeFrom: 'desc' } });
